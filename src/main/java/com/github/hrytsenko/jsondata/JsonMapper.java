...
public class JsonMapper<R extends JsonEntity<R>> {
   ...
   *
   * @param schema   the Jolt schema for the transformation. Use {@link JsonResources} to read the
   *                 Jolt schema.
   ...
   */
  public static <R extends JsonEntity<R>> JsonMapper<R> create(String schema,
      Supplier<R> supplier) {
  ...
  public R map(List<? extends JsonEntity<?>> entities) {
    return JsonExceptions.wrap(
        () -> JsonParser.mapToEntity(provider.mapObjects(JsonParser.entitiesToList(entities)),
            supplier),
        exception -> new JsonMapperException("Transformation failed", exception));
  }
